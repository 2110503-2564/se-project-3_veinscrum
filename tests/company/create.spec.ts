...
test("Create Company as admin", async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Company Name' }).fill('Playwright Company 234');
  await page.getByRole('textbox', { name: 'Address' }).click();
  await page.getByRole('textbox', { name: 'Address' }).fill('Floor 5th Room 508 Jumpi Hornai');
  await page.getByRole('textbox', { name: 'Website' }).click();
  await page.getByRole('textbox', { name: 'Website' }).fill('http://www.playwright.com');
  await page.getByRole('textbox', { name: 'editable markdown' }).getByRole('paragraph').click();
  await page.getByRole('textbox', { name: 'editable markdown' }).fill('This is the best company, Even room 508 bed A said it was great.');
